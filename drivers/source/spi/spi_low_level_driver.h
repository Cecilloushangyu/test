...
#ifndef _SPI_LOW_LEVEL_DRIVER_H_
#define _SPI_LOW_LEVEL_DRIVER_H_
...
typedef enum {
    SPI_ERROR_MODE  = (1000 + 1),     ///< Specified Mode not supported
...
/****** SPI Event *****/
typedef enum {
    SPI_EVENT_TRANSFER_COMPLETE = 0,   ///< Data Transfer completed. Occurs after call to _spi_send, _spi_receive, or _spi_transfer to indicate that all the data has been transferred. The driver is ready for the next transfer operation
    SPI_EVENT_TX_COMPLETE,              ///< Data Transfer completed. Occurs after call to _spi_send, _spi_receive, or _spi_transfer to indicate that all the data has been transferred. The driver is ready for the next transfer operation
    SPI_EVENT_RX_COMPLETE,              ///< Data Transfer completed. Occurs after call to _spi_send, _spi_receive, or _spi_transfer to indicate that all the data has been transferred. The driver is ready for the next transfer operation
...
typedef void (*spi_event_cb_t)(int32_t idx, spi_event_e event,void*arg);  ///< Pointer to \ref spi_event_cb_t : SPI Event call back.
...
*/
spi_handle_t dw_spi_initialize(int32_t idx,int32_t role, spi_event_cb_t cb_event,void* arg);
...
/**
  \brief         sending data to SPI transmitter,(received data is ignored).
               ...
               \ref dw_spi_get_status can get operation status.
...
/**
  ...
  \return      error code
*/
int32_t dw_spi_config_block_mode(spi_handle_t handle, int32_t flag);
